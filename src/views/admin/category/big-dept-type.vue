<template>
  <el-card class="custome">
    <div style="margin-bottom:10px;">
      <el-button size="small" type="primary" @click="handleAdd">添加大单位</el-button>
    ...
    <el-dialog v-el-drag-dialog :visible.sync="dialogVisible" :title="title" width="40%" @dragDialog="handleDrag">
      <el-form ref="unitForm" name="unitForm" :model="unitForm" label-position="right" label-width="120px" :rules="rules">
        <el-form-item label="大单位名称：" prop="name">
          <el-input v-model="unitForm.name" size="small" style="max-width:400px;" />
        </el-form-item>
        <el-form-item label="大单位标识：" prop="identity">
          <el-input v-model="unitForm.identity" size="small" style="max-width:400px;" />
        </el-form-item>
        <el-form-item label="上级大单位：" prop="parentId">
          <treeselect
    ...
    <div style="height: calc(100vh - 238px);overflow: auto">
      <el-table
        ...
        row-key="id"
        :header-cell-style="{background:'rgba(242, 242, 242, 0.5)',height:'30px !important'}"
      >
        <el-table-column align="center" header-align="center" width="55" type="index" label="序号" />
        <el-table-column align="left" header-align="left" label="大单位名称" prop="name" />
        <el-table-column align="center" header-align="center" label="大单位标识" prop="identity" />
...
export default {
  ...
  methods: {
     ...
     */
    getCategoryList() {
      category.getCategoryList("bigdept", this.currentPage, this.pageSize).then(response => {
        if (response.code === 200) {
    ...
    queryCategory() {
      ...
      if (this.search.keyWord) {
        category.queryCategory("bigdept", this.search.keyWord, this.currentPage, this.pageSize).then(response => {
          if (response.code === 200) {
            this.typeData = response.data.list
            this.totalCount = response.data.total
          }
