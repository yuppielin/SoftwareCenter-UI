<template>
  <el-card class="custome">
    <div style="margin-bottom:10px;">
      <el-button size="small" type="primary" @click="handleAdd">添加通知发布单位</el-button>
...
export default {
  ...
  methods: {
     ...
     */
    getCategoryList() {
      category.getCategoryList("notice_publish_dept", this.currentPage, this.pageSize).then(response => {
        if (response.code === 200) {
    ...
    queryCategory() {
      // 查询通知发布单位 7 为通知发布单位的根节点id
      if (this.search.keyWord) {
        category.queryCategory("notice_publish_dept", this.search.keyWord, this.currentPage, this.pageSize).then(response => {
          if (response.code === 200) {
            this.typeData = response.data.list
            this.totalCount = response.data.total
          }
