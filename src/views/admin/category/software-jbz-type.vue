    ...
    <div class="app-container">
        <el-card class="custome" >
            <div style="margin-bottom:10px;">
                <el-button size="small" type="primary" @click="handleAdd">添加兵种类型</el-button>
...
import * as category from '@/api/category'
 ...
 mounted() {
  ...
  methods: {
     ...
     */
    getCategoryList() {
      category.getCategoryList("army_cate", this.currentPage, this.pageSize).then(response => {
    ...
    queryCategory() {
      ...
      if (this.search.keyword) {
        category.queryCategory("army_cate", this.search.keyword, this.currentPage, this.pageSize).then(response => {
          if (response.code === 200) {
            this.typeData = response.data.list
            this.totalCount = response.data.total
          }
