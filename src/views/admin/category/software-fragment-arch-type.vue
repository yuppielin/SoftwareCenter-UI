    ...
    <div class="app-container">
        <el-card class="custome" >
            <div style="margin-bottom:10px;">
                <el-button size="small" type="primary" @click="handleAdd">添加CPU架构类型</el-button>
            ...
            :header-cell-style="{background:'#2BB56E',height:'30px'}" >
                ...
                <el-table-column align="center" header-align="left" label="分类名称" prop="name"></el-table-column>
...
import * as category from '@/api/category'
 ...
 mounted() {
  ...
  methods: {
     ...
     */
    getCategoryList() {
      category.getCategoryList("fragment_arch", this.currentPage, this.pageSize).then(response => {
    ...
    queryCategory() {
      ...
      if (this.search.keyword) {
        category.queryCategory("fragment_arch", this.search.keyword, this.currentPage, this.pageSize).then(response => {
          if (response.code === 200) {
            this.typeData = response.data.list
            this.totalCount = response.data.total
          }
