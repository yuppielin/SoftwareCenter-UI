<template>
  <el-card class="custome">
    ...
    <div style="height: calc(100vh - 238px);overflow: auto">
      ...
      >
        <el-table-column align="center" header-align="center" width="80" type="index" label="序号" />
...
export default {
  ...
  methods: {
     ...
     */
    getCategoryList() {
      category.getCategoryList("service", this.currentPage, this.pageSize).then(response => {
        if (response.code === 200) {
    ...
    queryCategory() {
      ...
      if (this.search.keyWord) {
        category.queryCategory("service", this.search.keyWord, this.currentPage, this.pageSize).then(response => {
          if (response.code === 200) {
            this.typeData = response.data.list
            this.totalCount = response.data.total
          }
